...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [user, setUser] = useState<UserData | null>(() => {
    try {
      const savedUser = localStorage.getItem('beThePeopleUser');
      if (savedUser) {
        const parsedUser = JSON.parse(savedUser);
        // Convert lastActiveDate back to Date object
        if (parsedUser.lastActiveDate) {
        ...
        if (parsedUser.achievements) {
          parsedUser.achievements = parsedUser.achievements.map((achievement: any) => ({
            ...achievement,
  ...
  React.useEffect(() => {
    if (user) {
      try {
        localStorage.setItem('beThePeopleUser', JSON.stringify(user));
      } catch (error) {
        console.error('Failed to save user to localStorage:', error);
      }
    } else {
      localStorage.removeItem('beThePeopleUser');
    }
  }, [user]);
  ...
  const incrementStreak = () => {
    if (user) {
      ...
      // Only update if we haven't already checked today
      if (daysDiff > 0) {
        ...
        const updatedUser = {
          ...user,
          stats: { ...user.stats, streak: newStreak },
